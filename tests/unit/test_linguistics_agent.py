...
# These imports will fail initially (RED phase) - this is expected
try:
...
class TestLinguisticsAgent:
    ...
    async def test_ebnf_processing_tool(self, sample_ebnf_grammar: str) -> None:
        ...
        with patch('linguistics_agent.agent.get_dependencies') as mock_deps:
            ...
            result = await agent.run_tool(
                'process_ebnf', 
                ebnf_content=sample_ebnf_grammar
            )
    ...
    async def test_agent_error_handling(self) -> None:
        ...
        with patch('linguistics_agent.agent.get_dependencies') as mock_deps:
            ...
            assert result.output.confidence_score < 0.5  # Low confidence for invalid input
    
    async def test_agent_context_preservation(self, sample_query: Dict[str, Any]) -> None:
...
class TestLinguisticsModels:
    ...
    def test_linguistics_query_validation(self) -> None:
        ...
        assert query.analysis_type == "syntactic"
        assert query.context["language"] == "english"
        
        # Invalid query - empty text
        with pytest.raises(ValueError, match="Text cannot be empty"):
            LinguisticsQuery(text="", analysis_type="syntactic")
    ...
    def test_linguistics_response_validation(self) -> None:
        ...
        valid_response_data = {
            "analysis_results": {"syntax": "valid", "semantics": "clear"},
            "grammar_insights": ["Subject-Verb-Object structure", "Present tense"],
            "recommendations": ["Consider passive voice", "Add complexity"],
            "confidence_score": 0.85
        }
        
        response = LinguisticsResponse(**valid_response_data)
        assert isinstance(response.analysis_results, dict)
        assert isinstance(response.grammar_insights, list)
