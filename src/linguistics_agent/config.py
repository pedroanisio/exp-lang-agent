...
class DatabaseConfig(PydanticBaseSettings):
    ...
    chromadb_collection: str = Field(default="linguistics_knowledge", env="CHROMADB_COLLECTION")
...
class TestConfig(PydanticBaseSettings):
    ...
    def validate_test_api_key(cls, v, values):
        ...
        if values.get('use_real_api') and not v:
            raise ValueError('TEST_ANTHROPIC_API_KEY required when TEST_USE_REAL_API=true')
        return v
...
class KnowledgeConfig(PydanticBaseSettings):
    ...
    storage_path: str = Field(default="/app/data/knowledge", env="KNOWLEDGE_STORAGE_PATH")
    max_file_size_mb: int = Field(default=50, env="MAX_FILE_SIZE_MB")
    allowed_file_types: List[str] = Field(default=["pdf", "txt", "md", "html"], env="ALLOWED_FILE_TYPES")
